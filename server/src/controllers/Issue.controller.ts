 ...
 * 投票问题
 */
export const voteIssue = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { id } = req.params;
    if (!id) {
      throw new Error('问题ID不能为空');
 ...
 * 获取问题统计
 */
export const getIssueStats = async (_req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
