...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [statsResponse, adminStatsResponse, popularPagesResponse, viewTrendsResponse] = await Promise.all([
        ...
        const formattedStats: StatData[] = [
          ...
          {
            title: '总浏览量',
            value: adminStats?.popularWorks?.reduce((sum, page) => sum + page.totalViews, 0) || 0,
            prefix: <EyeOutlined />,
            trend: TrendType.UP,
            trendValue: 0
          }
        ...
        // Map response data to match PopularPage interface
        const mappedPopularPages = (popularPagesResponse || []).map(page => ({
